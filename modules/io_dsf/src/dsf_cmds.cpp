            ...
            while (c.p < c.end)
            {
                ...
                case Cmd_NetworkChainRange:
                    if (!need(4, "u32 arg"))
                        return false;
                    c.skip(4);
                    if (opcode == Cmd_SetDefinition32)
                        ++out.def_selects;
                    if (opcode == Cmd_ObjectRange)
                        ++out.objects_placed;
                    if (opcode == Cmd_NetworkChainRange)
                        ++out.network_chains;
                    break;
                ...
                case Cmd_Object:
                    if (!need(2, "u16 arg"))
                        return false;
                    c.skip(2);
                    if (opcode == Cmd_SetDefinition16)
                        ++out.def_selects;
                    if (opcode == Cmd_Object)
                        ++out.objects_placed;
                    break;

                case Cmd_NetworkChain:
                {
                    if (!need(1, "NetworkChain count"))
                        return false;
                    const std::uint8_t n = c.u8();
                    if (!need(2u * n, "NetworkChain indices"))
                        return false;
                    c.skip(2u * n);
                    ++out.network_chains;
                    break;
                ...
                {
                    if (!need(1, "NetworkChain32 count"))
                        return false;
                    const std::uint8_t n = c.u8();
                    if (!need(4u * n, "NetworkChain32 indices"))
                        return false;
                    c.skip(4u * n);
                    ++out.network_chains;
                    break;
                ...
                case Cmd_Polygon:
                {
                    if (!need(3, "Polygon param+count"))
                        return false;
                    c.u16(); // param
                    const std::uint8_t n = c.u8();
                    if (!need(2u * n, "Polygon indices"))
                        return false;
                    c.skip(2u * n);
                    ++out.polygons;
                    break;
                ...
                {
                    if (!need(3, "NestedPolygon param+count"))
                        return false;
                    c.u16(); // param
                    std::uint8_t rings = c.u8();
                    while (rings--)
                    {
                        if (!need(1, "NestedPolygon ring count"))
                            return false;
                        const std::uint8_t n = c.u8();
                        if (!need(2u * n, "NestedPolygon ring indices"))
                            return false;
                        c.skip(2u * n);
                    }
                    ++out.polygons;
                ...
                {
                    if (!need(5, "NestedPolygonRange head"))
                        return false;
                    c.u16(); // param
                    const std::uint8_t n = c.u8();
                    c.u16(); // start
                    if (!need(2u * n, "NestedPolygonRange ring offsets"))
                        return false;
                    c.skip(2u * n);
                    ++out.polygons;
                    break;
                ...
                case Cmd_TerrainPatchFlagsLOD:
                    if (!need(8, "TerrainPatchFlagsLOD near/far"))
                        return false;
                    c.skip(8);
                    [[fallthrough]];
                case Cmd_TerrainPatchFlags:
                    if (!need(1, "TerrainPatchFlags"))
                        return false;
                    c.skip(1);
                    [[fallthrough]];
                ...
                {
                    if (!need(1, "Triangle* count"))
                        return false;
                    const std::uint8_t n = c.u8();
                    if (!need(2u * n, "Triangle* indices"))
                        return false;
                    c.skip(2u * n);
                ...
                {
                    if (!need(1, "Triangle*CrossPool count"))
                        return false;
                    const std::uint8_t n = c.u8();
                    if (!need(4u * n, "Triangle*CrossPool tuples"))
                        return false;
                    c.skip(4u * n);
                    out.triangles_emitted += 1;
                    out.triangle_vertices += n;
                ...
                case Cmd_Comment8:
                {
                    if (!need(1, "Comment8 length"))
                        return false;
                    const std::uint8_t n = c.u8();
                    if (!need(n, "Comment8 body"))
                        return false;
                    c.skip(n);
                    break;
                }
                case Cmd_Comment16:
                {
                    if (!need(2, "Comment16 length"))
                        return false;
                    const std::uint16_t n = c.u16();
                    if (!need(n, "Comment16 body"))
                        return false;
                    c.skip(n);
                    break;
                }
                case Cmd_Comment32:
                {
                    if (!need(4, "Comment32 length"))
                        return false;
                    const std::uint32_t n = c.u32();
                    if (!need(n, "Comment32 body"))
                        return false;
                    c.skip(n);
                    break;
                }
