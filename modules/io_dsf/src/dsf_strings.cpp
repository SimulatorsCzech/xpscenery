...
namespace xps::io_dsf
{
    ...
    read_properties(const std::filesystem::path &dsf_path)
    {
        ...
        if (!strings)
            return std::unexpected(std::move(strings.error()));
    ...
    read_defn_strings(const std::filesystem::path &dsf_path, DefnKind kind)
    {
        ...
        switch (kind)
        {
        case DefnKind::terrain_types:
            child_id = kTertId;
            break;
        case DefnKind::object_defs:
            child_id = kObjtId;
            break;
        case DefnKind::polygon_defs:
            child_id = kPolyId;
            break;
        case DefnKind::network_defs:
            child_id = kNetwId;
            break;
        case DefnKind::raster_names:
            child_id = kDemnId;
            break;
        }
