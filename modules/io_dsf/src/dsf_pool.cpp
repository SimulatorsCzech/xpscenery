                    ...
                    while (i < plane_size)
                    {
                        ...
                        if (code & 0x80)
                        {
                            ...
                            for (std::uint32_t k = 0; k < len && i < plane_size; ++k, ++i)
                            {
                                ...
                                if (differenced)
                                {
                                    out_v = static_cast<T>(last + out_v);
                                    last = out_v;
                                }
    ...
    read_point_pools(const std::filesystem::path &dsf_path)
    {
        ...
        std::vector<TopLevelAtom> kids;
        {
            ...
            while (c + 8 <= child_end)
            {
                ...
                const std::uint32_t id = (static_cast<std::uint32_t>(std::to_integer<unsigned>(h[3])) << 24) | (static_cast<std::uint32_t>(std::to_integer<unsigned>(h[2])) << 16) | (static_cast<std::uint32_t>(std::to_integer<unsigned>(h[1])) << 8) | static_cast<std::uint32_t>(std::to_integer<unsigned>(h[0]));
                std::uint32_t sz;
        ...
        for (const auto &c : kids)
        {
            if (c.raw_id == kPoolId)
                pools16.push_back(&c);
            else if (c.raw_id == kScalId)
                scales16.push_back(&c);
            else if (c.raw_id == kPo32Id)
                pools32.push_back(&c);
            else if (c.raw_id == kSc32Id)
                scales32.push_back(&c);
        }
                              ...
                              bool is_32) -> std::expected<PointPool, std::string>
        {
            ...
            if (!scalers)
                return std::unexpected(std::move(scalers.error()));
        ...
        for (std::size_t i = 0; i < pools16.size(); ++i)
        {
            ...
            if (!pp)
                return std::unexpected(std::move(pp.error()));
