                 ...
                 std::string_view& key, std::string_view& rest) noexcept
{
    ...
    if (n == std::string_view::npos) { key = line; rest = {}; return; }
    key  = line.substr(0, n);
                    ...
                    Platform& plat, std::uint32_t& version) noexcept
{
    ...
    while (!cursor.empty()) {
        ...
        if (phase == 0) {
            if (line == "A") plat = Platform::apple;
            else if (line == "I") plat = Platform::ibm;
            else return false;
            ++phase;
        } else if (phase == 1) {
            if (!parse_u32(line, version)) return false;
            ++phase;
        } else {
...
read_obj_info(const std::filesystem::path& path)
{
    ...
    if (!parse_preamble(text, info.platform, info.version)) {
        return std::unexpected(std::format(
            ...
            "(expected A|I / version / OBJ)", path.string()));
    }
    ...
    while (!text.empty()) {
        ...
        if      (key == "TEXTURE")           info.texture        = std::string{rest};
        else if (key == "TEXTURE_LIT")       info.texture_lit    = std::string{rest};
        else if (key == "TEXTURE_NORMAL")    info.texture_normal = std::string{rest};
        ...
        else if (key == "TRIS")       ++info.tris_commands;
        else if (key == "LINES")      ++info.lines_commands;
        else if (key == "LIGHTS")     ++info.lights_commands;
        else if (key == "ANIM_begin") ++info.anim_begin;
    }
